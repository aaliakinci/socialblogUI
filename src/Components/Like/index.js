...
useEffect(() => {
	fetchIsLike();
	fetchLikes();
}, [article_id,user_id,isLike,setUserIsLike])
...
const handleClick = async () => {
	...
	return (
		<div className="col-12">
			{userIsLike === false ? (
				<span>
					<i className="fa fa-heart" onClick={handleClick}></i>{likeCount} 
				</span>
			) : (
				<span>
					<i className="fa fa-heart" onClick={handleClick} style={{ color: 'red' }}></i>{likeCount} 
				</span>
			)}
		</div>
	);
